...
def LF_experiment(
    ...
    for rep in range(reps):
        ...
        _tmp_ = np.zeros((28,28), dtype=int)
        ...
        train_x1 = train_x1.reshape(
            (
                train_x1.shape[0],
                train_x1.shape[1] * train_x1.shape[2],
            )
        )
        tmp_data = tmp_data.reshape(
            (
                tmp_data.shape[0],
                tmp_data.shape[1] * tmp_data.shape[2],
            )
        )
...
def image_aug(pic, angle, centroid_x=23, centroid_y=23, win=14, scale=1.5):
    # Calculates scaled dimensions of image
    im_sz = int(np.floor(pic.shape[0] * scale))
    pic_ = np.uint8(np.zeros((im_sz, im_sz), dtype=int))

    # Uses zoom function from scipy.ndimage to zoom into the image
    pic_[:, :] = ndimage.zoom(pic[:, :], scale)
