...
def fte_bte_experiment(
    ...
    for task_num in range((which_task - 1), 10):
        ...
        if task_num == (which_task - 1):
            learner.add_task(
                X=train_x[(task_num * 90) : ((task_num + 1) * 90)],
                y=train_y[(task_num * 90) : ((task_num + 1) * 90)],
                task_id=0,
            )
            ...
            while t_num < task_num:
                ...
                llf_task = learner.predict(
                    test_x[((which_task - 1) * 120) : (which_task * 120), :], task_id=0
                )
                acc = np.mean(
                ...
                learner.add_transformer(
                    X=train_x[(t_num * 90) : ((t_num + 1) * 90)],
                    y=train_y[(t_num * 90) : ((t_num + 1) * 90)],
                )
