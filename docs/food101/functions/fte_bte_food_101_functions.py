...
def cross_val_data(data_x, data_y, num_points_per_task, total_cls=100):
    ...
    for i in range(total_cls):
        ...
        if i == 0:
            ...
            test_x = x[indx[num_points_per_task:1000], :]
            test_y = y[indx[num_points_per_task:1000]]
        else:
            # num_points_per_task training data points per task
            ...
            # 1000 testing data points per task
            test_x = np.concatenate((test_x, x[indx[num_points_per_task:1000], :]), axis=0)
            test_y = np.concatenate((test_y, y[indx[num_points_per_task:1000]]), axis=0)
...
def label_shuffle_experiment(
    ...
    for task_num in range(10):
        ...
        if task_num == 0:
            rand_idx = np.random.randint(0, 9000, 900)
            #print("Added task")
            learner.add_task(
                X = train_x[rand_idx, :],
                y = train_y[rand_idx],
